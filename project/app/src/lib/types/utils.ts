...
export namespace Utils {
	...
	export type Join<T extends any[], D extends string> =
		T extends [] ? ""
		: T extends [infer F extends string] ? `${F}`
		: T extends [infer F extends string, ...infer R] ? `${F}${D}${Join<R, D>}`
		: string;
	...
	export type IsBijective<
		...
		result = {
			[K in keyof inverted]: Utils.TuplifyUnion<inverted[K]>["length"] extends 1 ? true
